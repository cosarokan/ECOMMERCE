...
namespace ECOMMERCE.WEBUI.Logging
{
    public class Logger : ILogger
    {
        ...
        public async void Log<TState>(LogLevel logLevel, EventId eventId, TState state, Exception exception, Func<TState, Exception, string> formatter)
        {
            ...
            {
                await streamWriter.WriteLineAsync($"Date: {DateTime.Now.ToString()} | Log Level : {logLevel.ToString()} | Event ID : {eventId.Id} | Event Name : {eventId.Name} | Formatter : {formatter(state, exception)}");
                streamWriter.Close();
